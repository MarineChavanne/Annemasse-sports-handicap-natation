...
import { genererGrilleMois, nomMois, nomJourCourt, toISODate, formatDateFr, formatHeure, todayISO } from '../lib/format'
...
export default function Presences() {
  ...
  const [modalAppel, setModalAppel] = useState(null) // jour sélectionné pour l'appel
  const [etatAppel, setEtatAppel] = useState({}) // { participant_id: true/false }
  ...
  return (
    <div>
      <ViewHeader
        title="Présences"
        subtitle="Cliquez sur un jour du calendrier pour faire l'appel, ou sur un jour vide pour planifier un entraînement"
        action={<Button onClick={ouvrirAjoutJour}>+ Planifier un entraînement</Button>}
      />
      ...
      <div className="card">
        ...
        {loading ? (
          ...
          <EmptyState icon="🗓️" title="Aucun entraînement planifié" description="Ajoutez un jour d'entraînement pour commencer à faire l'appel." />
        ) : (
          <div>
            {joursAVenir.map((j) => (
