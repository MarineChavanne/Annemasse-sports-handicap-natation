...
const JOUR_VIDE = { date: '', label: 'Entraînement', heure_debut: '', heure_fin: '' }
...
const IMPORT_MASSE_VIDE = { date_debut: '', date_fin: '', joursSemaine: [], label: 'Entraînement', heure_debut: '', heure_fin: '' }
...
export default function Presences({ isEntraineur = false }) {
  ...
  function clicSurJour(cellule) {
    ...
    if (jourExistant) {
      ...
      setFormJour({ date: cellule.iso, label: 'Entraînement', heure_debut: '', heure_fin: '' })
  ...
  function ouvrirEditionJour(j) {
    setFormJour({ date: j.date, label: j.label || '', heure_debut: j.heure_debut || '', heure_fin: j.heure_fin || '' })
    setJourEnEdition(j)
    setModalNouveauJour(true)
  }
  ...
  async function lancerImportMasse(e) {
    ...
    try {
      const lignes = apercu.map((iso) => ({
        ...
        heure_debut: formImportMasse.heure_debut || null,
        heure_fin: formImportMasse.heure_fin || null,
      }))
              ...
              <div key={j.id} className="list-row">
                <div>
                  ...
                  <div style={{ color: 'var(--texte-doux)', fontSize: 13 }}>
                    {j.label}{formatPlageHoraire(j.heure_debut, j.heure_fin) ? ` — ${formatPlageHoraire(j.heure_debut, j.heure_fin)}` : ''}
                  </div>
                </div>
          ...
          <form onSubmit={enregistrerJour}>
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                <input type="time" value={formJour.heure_debut} onChange={(e) => setFormJour({ ...formJour, heure_debut: e.target.value })} />
              </div>
              <div className="form-group">
                <label>Heure de fin (optionnel)</label>
                <input type="time" value={formJour.heure_fin} onChange={(e) => setFormJour({ ...formJour, heure_fin: e.target.value })} />
              </div>
            </div>
          </form>
        </Modal>
          ...
          <form onSubmit={lancerImportMasse}>
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                <input
                  type="time"
                  value={formImportMasse.heure_debut}
                  onChange={(e) => setFormImportMasse({ ...formImportMasse, heure_debut: e.target.value })}
                />
              </div>
              <div className="form-group">
                <label>Heure de fin (optionnel)</label>
                <input
                  type="time"
                  value={formImportMasse.heure_fin}
                  onChange={(e) => setFormImportMasse({ ...formImportMasse, heure_fin: e.target.value })}
                />
              </div>
            </div>
